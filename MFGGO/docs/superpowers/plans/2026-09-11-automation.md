...
Create automation-ui.js and automation.css. Exports renderAutomation(store), mountAutomation(host,store,{apiRequest,showToast}) returning disposer. Owner-only navigation independent of a new billable module. GET snapshot on mount; CRUD via API; preserve unsaved text on errors, detect revisions, cancel requests on dispose. Textarea source editor with line count, insert-at-cursor palette and compiler errors. Left event CRUD/order/enable. Collections in a separate compact dialog use organization members and department labels. Runs in compact dialog. AI response shown as editable draft with apply/cancel; never autosave/enable.

## Task 3: integration and verification

Run node --test tests/automation.test.mjs plus focused executor/history regressions; npm run build. Review backend and UI constraints against changed files. Keep credentials/server setup limitation explicit. Record commands/results and open enterprise preview.
